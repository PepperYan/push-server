...
class RedisHelper {
  ...
  getBuffer(key,callback){
    return this.client.getAsync(new Buffer(key)).then((reply)  => {
  ...
  hget(hashTable, key, callback) {
    return this.client.hgetAsync(hashTable, key).then((reply) =>  {
        callback(null,reply);
    })
    .catch((err) => {
  ...
  hdel(hashTable, key, callback) {
    this.client.hdelAsync(hashTable, key).then((reply)  => {
